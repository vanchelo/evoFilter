...
class phpFastCache
{
    ...
    /**
     * Hour | auto clean up files after this
     *
     * @var int
     */
    public static $filesCleanupAfter = 1;
    /**
     * @var int
     */
     ...
     * @var string
     */
    public static $securityKey = "custom"; // phpFastCache::$securityKey = "newKey";
    /**
     * @var bool
     */
    ...
    /**
     * Use for get from Tmp Memory, will be faster in checking cache on LOOP.
     *
     * @var bool
     */
    public static $useTmpCache = false;
    /**
     * @var bool
     */
     ...
     * @var int
     */
    private static $stepDebugging = 0;
    /**
     * @var array
     */
    private static $Tmp = [];
...
allow from 127.0.0.1";
    ...
    public static function autoconfig($name = "")
    {
        ...
        if (self::$storage == "" || self::$storage == "auto") {
            ...
            if (in_array(self::$storage, ["files", "pdo", "mpdo"])) {
                self::$sys['storage'] = "disk";
            } elseif (in_array(self::$storage, ["apc", "memcache", "memcached", "wincache", "xcache"])) {
                self::$sys['storage'] = "memory";
            } else {
                self::$sys['storage'] = "";
            }

            if (self::$sys['storage'] == "" || !in_array(self::$storage, self::$supportedApi)) {
        ...
        if (self::$sys['method'] == "files") {
            ...
            if ($last_cleanup == null) {
                self::files_cleanup();
                self::files_set("last_cleanup_cache", @date("U"), 3600 * self::$filesCleanupAfter);
            }
        }
    ...
    private static function cacheMethod($name = "")
    {
        ...
        if (is_array($name)) {
            $key = array_keys($name);
            $key = $key[0];
            if (in_array($key, self::$supportedApi)) {
                $cache = $key;
            }
        }
     ...
     * @return array
     */
    public static function deleteMulti($object = [])
    {
        $res = [];
    ...
    /**
     * @param array $mname
     * @param int   $time_in_second_for_all
     * @param bool  $skip_for_all
     *
     * @return array
     */
    public static function setMulti($mname = [], $time_in_second_for_all = 600, $skip_for_all = false)
    {
        $res = [];
        ...
        foreach ($mname as $object) {
            ...
            if ($keys[0] != "0") {
                $k = $keys[0];
                $name = isset($object[$k]) ? [$k => $object[$k]] : "";
                $n = $k . "_" . $object[$k];
                $x = 0;
            } else {
     ...
     * @return array
     */
    public static function getMulti($object = [])
    {
        $res = [];
        foreach ($object as $driver => $name) {
            if (!is_numeric($driver)) {
     ...
     * @return bool
     */
    private static function xcache_cleanup($option = [])
    {
        // Revision 621
     ...
     * @return bool
     */
    private static function apc_cleanup($option = [])
    {
    ...
    private static function pdo_exist($name)
    {
        ...
        $x = self::db(['db' => $db['db']])->prepare("SELECT COUNT(*) as `total` FROM " . self::$table . " WHERE `name`=:name");
    ...
    {
        self::db(["skip_clean" => true])->exec("drop table if exists " . self::$table);
        self::initDatabase();
    ...
    private static function pdo_get($name)
    {
        ...
        $stm = self::db(['db' => $db['db']])->prepare("SELECT * FROM " . self::$table . " WHERE `name`='" . $name . "'");
        $stm->execute();
    ...
    private static function pdo_decrement($name, $step = 1)
    {
        ...
        $int = self::get($name);
        try {
            $stm = self::db(['db' => $db['db']])->prepare("UPDATE " . self::$table . " SET `value`=:new WHERE `name`=:name ");
            $stm->execute([
    ...
    private static function pdo_delete($name)
    {
        ...
        return self::db(['db' => $db['db']])->exec("DELETE FROM " . self::$table . " WHERE `name`='" . $name . "'");
    }
    ...
    private static function pdo_set($name, $value, $time_in_second = 600, $skip_if_existing = false)
    {
        ...
        if ($skip_if_existing == true) {
            try {
                $insert = self::db(['db' => $db['db']])->prepare("INSERT OR IGNORE INTO " . self::$table . " (name,value,added,endin) VALUES(:name,:value,:added,:endin)");
                try {
            ...
            try {
                $insert = self::db(['db' => $db['db']])->prepare("INSERT OR REPLACE INTO " . self::$table . " (name,value,added,endin) VALUES(:name,:value,:added,:endin)");
                try {
                    $value = self::encode($value);
                } catch (Exception $e) {
                    die("Sorry! phpFastCache don't allow this type of value - Name: " . $name);
     ...
     * @return mixed
     */
    private static function db($option = [])
    {
        $vacuum = false;
    ...
    {
        if ($object == null) {
            self::db(["skip_clean" => true])->exec('CREATE TABLE IF NOT EXISTS "' . self::$table . '" ("id" INTEGER PRIMARY KEY  AUTOINCREMENT  NOT NULL  UNIQUE , "name" VARCHAR UNIQUE NOT NULL  , "value" BLOB, "added" INTEGER NOT NULL  DEFAULT 0, "endin" INTEGER NOT NULL  DEFAULT 0)');
            self::db(["skip_clean" => true])->exec('CREATE INDEX "lookup" ON "' . self::$table . '" ("added" ASC, "endin" ASC)');
            self::db(["skip_clean" => true])->exec('VACUUM');
        } else {
    ...
    public static function startDebug($value, $text = "", $line = __LINE__, $func = __FUNCTION__)
    {
        if (self::$debugging == true) {
            self::$stepDebugging++;
            if (!is_array($value)) {
                echo "<br>" . self::$stepDebugging . " => " . $line . " | " . $func . " | " . $text . " | " . $value;
            } else {
                echo "<br>" . self::$stepDebugging . " => " . $line . " | " . $func . " | " . $text . " | ";
                print_r($value);
            }
        }
    }
